...
async function checkSession() {
    // Sesi login tetap disimpan di local browser agar user tidak perlu login terus
    ...
    if (currentUser) {
        ...
        // Tarik poin terbaru dari Supabase
...
async function login() {
    ...
    // Baca Database: SELECT * FROM users WHERE email = ... AND password = ...
...
async function register() {
    ...
    if (error) {
        alert("Gagal daftar! Email mungkin sudah digunakan.");
    } else {
...
// ==========================================
// FITUR GPS & AI (Tetap berjalan di client-side)
// ==========================================
...
// ==========================================
// CRUD UTAMA (SIMPAN & BACA LAPORAN)
// ==========================================
...
async function submitLaporan(e) {
    ...
    // Tulis Database: INSERT INTO laporan
    const btn = document.getElementById('btn-submit-laporan');
...
// ==========================================
// KLAIM & SERAH TERIMA (LOGIKA RELASIONAL)
// ==========================================
async function openDetail(id) {
    ...
    // Ambil data Laporan dan Klaim terkait menggunakan Supabase
    ...
    if (currentUser) {
        ...
        if (item.status === 'Aktif') {
            if (isPembuatPos) {
                ...
                klaimList.filter(k => k.status === 'PENDING').forEach(k => {
                    listHTML += `
                        <div class="action-box">
                            <h4>Tanggapan dari: ${k.claimer_nama}</h4>
                            <p>"${k.pesan}"</p>
                ...
                actionBox = `
                    <div class="detail-box">
                        ...
                        <textarea id="klaim-pesan" class="claim-input" placeholder="Sebutkan detail barang..."></textarea>
                        <button class="btn-blue-full" onclick="ajukanKlaim(${item.id}, ${item.user_id}, '${item.judul}')">${btnText}</button>
                    </div>`;
            }
        } 
            ...
            actionBox = `
                <div class="detail-box" style="border-color: var(--yellow);">
                    ...
                    <p>Dititipkan di: <strong>${klaimDiterima.drop_point}</strong></p>
                    ${isPemilikAsli ? `<button class="btn-yellow-full" onclick="selesaikanLaporan(${item.id}, ${idPenemu})">Konfirmasi Barang Diterima & Beri Poin Kejujuran</button>` : `<p style="margin-top:10px; color:var(--green);">Menunggu pemilik asli mengambil barang.</p>`}
                </div>`;
        }
        ...
        actionBox = `<div class="detail-box"><p><a href="#" onclick="navigate('login')">Masuk</a> untuk berinteraksi.</p></div>`;
    }
...
async function terimaKlaim(klaimId, laporanId) {
    ...
    // Ambil ID Claimer untuk dikirimi notif
...
async function selesaikanLaporan(laporanId, idPenemu) {
    ...
    // Update Poin Penemu di Tabel Users (Menggunakan query RPC / Math sederhana)
...
async function renderNotif() {
    ...
    myNotif.forEach(n => {
        container.innerHTML += `<div class="notif-card"><div><p>${n.pesan}</p><a class="notif-link" onclick="openDetail(${n.laporan_id})">Lihat Detail →</a></div></div>`;
    });
